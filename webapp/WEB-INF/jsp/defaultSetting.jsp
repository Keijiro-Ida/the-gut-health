	...
	<form action="/the-gut-healthy/DefaultSettingServlet" method="post" id="my_form">
		<table>
			...
			<% } else { %>
				<% for(int j = 0; j < 12; j += 2) {%>
					<tr>
						<th><%=threeMealsDAO.selectThreeMeals().get(j).getThreeMealsName() %></th>
								<td>
								<input type="time" name="meal_time<%=j+1 %>"  form="my_form" />
								</td>
								<td>
		...
		<button type="submit" name="defaultSubmit" value="0" form="my_form" >削除</button>
		<button type="submit" name="defaultSubmit" value="1" form="my_form" >登録</button>
	</form>
